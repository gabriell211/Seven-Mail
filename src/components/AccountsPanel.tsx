          ...
          <section className="modal account-edit-modal" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <div className="form-grid account-edit-grid">
              ...
              {editing.isSharedMailbox&&<>
                ...
                <label className="full inline-check"><input type="checkbox" checked={Boolean(editing.autoReplyEnabled)} onChange={(event)=>setEditing({...editing,autoReplyEnabled:event.target.checked})}/> Resposta automática exclusiva desta caixa</label>
                {editing.autoReplyEnabled&&<>
                  <label className="full"><span>Assunto da resposta</span><input value={editing.autoReplySubject??"Resposta automática"} onChange={(event)=>setEditing({...editing,autoReplySubject:event.target.value})}/></label>
                  <label className="full"><span>Mensagem automática</span><textarea value={editing.autoReplyBody??""} onChange={(event)=>setEditing({...editing,autoReplyBody:event.target.value})}/></label>
                  <label><span>Início</span><input type="datetime-local" value={editing.autoReplyStart?.slice(0,16)??""} onChange={(event)=>setEditing({...editing,autoReplyStart:event.target.value||undefined})}/></label>
                  <label><span>Fim</span><input type="datetime-local" value={editing.autoReplyEnd?.slice(0,16)??""} onChange={(event)=>setEditing({...editing,autoReplyEnd:event.target.value||undefined})}/></label>
                </>}
              </>}
