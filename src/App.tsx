...
function SettingsView({settings,onChange,runtime,accounts,onAccountsChange,signatures,onSaveSignature,onDeleteSignature,profiles,activeProfileId,onActivateProfile,onSaveProfile,onDeleteProfile,onUpdateInfo}:{settings:AppSettings;onChange:(s:AppSettings)=>void;runtime?:RuntimeInfo;accounts:AccountProfile[];onAccountsChange:(accounts:AccountProfile[])=>void;signatures:SignatureItem[];onSaveSignature:(signature:SignatureItem)=>Promise<void>;onDeleteSignature:(signature:SignatureItem)=>Promise<void>;profiles:ProfileItem[];activeProfileId?:string;onActivateProfile:(profile:ProfileItem|null)=>void;onSaveProfile:(profile:ProfileItem)=>Promise<void>;onDeleteProfile:(profile:ProfileItem)=>Promise<void>;onUpdateInfo:(info:UpdateInfo)=>void}) {
  ...
  function chooseMigrationAccount(): AccountProfile | undefined {
    if(accounts.length===0){
      window.alert("Adicione uma conta antes de importar arquivos de migração.");
      return undefined;
    }
    if(accounts.length===1) return accounts[0];
    const email=window.prompt(
      "Conta que receberá os dados importados",
      accounts.find((account)=>account.isDefault)?.email??accounts[0].email,
    )?.trim();
    if(!email) return undefined;
    const account=accounts.find((item)=>item.email.toLocaleLowerCase("pt-BR")===email.toLocaleLowerCase("pt-BR"));
    if(!account) window.alert("Conta não encontrada.");
    return account;
  }

  async function importPstArchive() {
    const account=chooseMigrationAccount();
    if(!account) return;
    const selected=await open({
      multiple:false,
      directory:false,
      filters:[{name:"Arquivo PST",extensions:["pst"]}],
    });
    if(!selected||Array.isArray(selected)) return;
    const count=await bridge.importPst(account.id,selected);
    window.alert(count===1?"1 mensagem importada do PST.":`${count} mensagens importadas do PST.`);
    window.location.reload();
  }

  async function exportPstArchive() {
    const destination=await saveDialog({
      defaultPath:"seven-mail-mailbox.pst",
      filters:[{name:"Arquivo PST",extensions:["pst"]}],
    });
    if(!destination) return;
    const count=await bridge.exportPst(undefined,destination);
    window.alert(count===1?"1 mensagem exportada para PST.":`${count} mensagens exportadas para PST.`);
  }
...
export default function App() {
  ...
  async function recallMessage(message: MailMessage) {
    if(!window.confirm("Solicitar ao provedor o recolhimento desta mensagem enviada? O sucesso depende do servidor, destinatários e políticas da organização.")) return;
    try{
      const result=await bridge.providerRecallMessage(message.accountId,message.id);
      window.alert(result);
    }catch(reason){
      window.alert(reason instanceof Error?reason.message:String(reason));
    }
  ...
  async function redirectMessage(message: MailMessage) {
    const target=window.prompt("Redirecionar para qual e-mail?")?.trim();
    if(!target) return;
    ...
    const staged=await bridge.stageMessageAsEml(
      operationId,
      message.accountId,
      message.id,
      safeExportName(message.subject,"mensagem"),
    );
    await bridge.queueOperation({
      id:operationId,
      kind:"redirect",
    ...
    void bridge.flushOutbox().catch(()=>undefined);
    window.alert("Redirecionamento adicionado à caixa de saída.");
  }
  ...
  return <><LaunchScreen ready={bootReady}/>{appLocked&&<AppLockScreen onUnlock={unlockApp}/>}<a className="skip-link" href="#seven-mail-main">Pular para o conteúdo</a><div className="app-shell">
    ...
    <main className="main" id="seven-mail-main" tabIndex={-1}>
